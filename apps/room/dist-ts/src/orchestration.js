import { discoverNativeTools, executeNativeTool, hasNativeWebMCP, } from "@relayroom/ui";
export class CrossOriginToolClient {
    ...
    async setSession(tokens, order) {
        this.session = { tokens, order };
        await Promise.all(Object.keys(this.origins).map((partner) => this.sendSession(partner)));
    }
    sendSession(partner) {
        if (!this.session)
    ...
    async discover() {
        ...
        if (hasNativeWebMCP()) {
            try {
                ...
                const native = (await discoverNativeTools(requestedOrigins)).filter((tool) => requestedOrigins.includes(tool.origin));
                this.nativeTools = new Map(native.map((tool) => [tool.name, tool]));
                if (requestedOrigins.every((origin) => native.some((tool) => tool.origin === origin)))
                    return { tools: native, mode: "native" };
            }
            catch (error) {
                console.info("[RelayRoom] Native discovery did not return partner tools.", error);
            }
        }
        ...
        Object.keys(frames).forEach((partner) => {
            frames[partner]?.contentWindow?.postMessage({ type: "relayroom:discover", requestId }, this.origins[partner]);
        });
    ...
    async execute(toolName, input, signal) {
        ...
        if (native)
            return executeNativeTool(native, input, signal);
        ...
        if (!targetWindow)
            throw new Error(`${partner} portal is unavailable`);
        const requestId = crypto.randomUUID();
        return new Promise((resolve, reject) => {
            if (signal?.aborted)
            ...
            targetWindow.postMessage({ type: "relayroom:execute", requestId, tool: toolName, input }, tool.origin);
        });
    }
    broadcastPhase(phase) {
        const frames = this.frames();
        Object.keys(frames).forEach((partner) => {
            frames[partner]?.contentWindow?.postMessage({ type: "relayroom:phase", phase }, this.origins[partner]);
        });
    }
    clearNativeCache() {
        this.nativeTools.clear();
    ...
    handleMessage(event) {
        if (!Object.values(this.origins).includes(event.origin))
            return;
        const partner = this.partnerForOrigin(event.origin);
        const sourceIsTrustedFrame = this.frames()[partner]?.contentWindow === event.source;
        if (!sourceIsTrustedFrame)
            return;
        ...
        if (message.type === "relayroom:tools" && Array.isArray(message.tools)) {
            message.tools.forEach((tool) => this.bridgeTools.set(tool.name, { ...tool, origin: event.origin }));
        }
        if (message.type === "relayroom:result" && message.requestId) {
            ...
            if (message.ok)
                ...
                pending.reject(new Error(message.error?.message || "Partner tool failed"));
        }
    }
    partnerForOrigin(origin) {
        const match = Object.entries(this.origins).find(([, value]) => value === origin);
        if (!match)
            throw new Error(`Untrusted partner origin: ${origin}`);
        return match[0];
    }
}
