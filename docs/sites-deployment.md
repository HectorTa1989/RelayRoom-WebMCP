...
5. the room static Site (`dist`, generated from `apps/room`).

The API and portals can remain on the existing Docker/Node host. The room Site can then be published from the root `dist` folder generated by `npm run build:site`. The origins are compiled into the room bundle, so set all four values before building:
